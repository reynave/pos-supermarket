...
router.post('/new', validate(createCartSchema), cartController.createCart);

// GET /api/cart/list/:kioskUuid — get cart items + totals (must be before /:kioskUuid)
router.get('/list/:kioskUuid', cartController.listCart);

// POST /api/cart/void/:kioskUuid — verify PIN + hard delete all cart data
router.post('/void/:kioskUuid', cartController.voidCart);

// GET /api/cart/:kioskUuid — get cart session info
router.get('/:kioskUuid', cartController.getCart);

// Buatkan void item post, body isi nya kioskUuid, itemId/barcode, qty, reason
router.post('/voidItem/:kioskUuid', cartController.voidCartItem);
